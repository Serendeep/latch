CREATE TEMP TABLE copy_audit_down_guard (allowed INTEGER CHECK(allowed=1));
INSERT INTO copy_audit_down_guard SELECT NOT EXISTS(SELECT 1 FROM audit_events WHERE operation=13);
DROP TABLE copy_audit_down_guard;
