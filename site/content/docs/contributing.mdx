...
Follow the [getting started guide](./getting-started.mdx) to install prerequisites and start the desktop. Run commands below from the repository root. Use pnpm and the tool versions in `mise.toml`.
...
The [release-please](https://github.com/googleapis/release-please-action) workflow maintains the version and changelog in a release PR. Merging that PR creates a draft GitHub release and builds the CLI and desktop packages on Linux, macOS, and Windows. Drafts must stay unpublished until their artifacts have passed platform testing and signing review.

Build unsigned native packages locally with `pnpm desktop:bundle`. Use `pnpm desktop:bundle --bundles deb,appimage` for Linux. On Arch Linux, set `NO_STRIP=1` when linuxdeploy cannot process newer RELR sections. Signing, automatic updates, and CLI installation are not connected yet.
...
Edit `site/content/docs/` in the same pull request as behavior changes. Install the site dependencies with `pnpm --dir site install --frozen-lockfile`, preview with `pnpm docs:dev`, and build with `pnpm docs:build`. Keep unreleased limitations explicit. GitHub Pages publishes the static output in `site/out/` at `latch.serendeep.tech`.
